...
export default class ScaleElement {
  constructor(cssSelector, options) {
    ...
    if (parentWidth <= 0 || parentHeight <= 0) {
      throw new Error(
        "parentWidth parentHeight 必须大于0，如果不传则取 document.body.clientWidth document.body.clientHeight"
      );
    }
    ...
    if (
      minScale > 0 &&
      maxScale > 0 &&
      (defaultScale < minScale || defaultScale > maxScale)
    ) {
  ...
  init() {
    ...
    function _onPan(ev) {
      ...
      const { width, height } = that._element.getBoundingClientRect();
